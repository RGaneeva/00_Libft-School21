...
/*   Updated: 2021/05/08 20:09:34 by pcarmeli         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	**ft_split(char const *s, char c)
{
	...
	if (!s)
		return (0);
	wordcount = ft_words_in_str(s, c);
	result = malloc(sizeof(char *) * (wordcount + 1));
	if (!result)
		return (0);
	result = ft_arrayfill(s, wordcount, c, result);
	return (result);
}
/*allocates and returns an array of strings */
/*obtained by spliting string using char c */
/* as delimiter. array ends by 0 pointer */
